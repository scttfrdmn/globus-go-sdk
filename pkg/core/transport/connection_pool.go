...
// ConnectionPoolTransport wraps an HTTP transport with connection pool management
type ConnectionPoolTransport struct {
	// Pool is the connection pool to use
	Pool *ConnectionPool

	// Underlying is the underlying HTTP transport
	Underlying *http.Transport
}

// NewConnectionPoolTransport creates a new transport that uses the given connection pool
func NewConnectionPoolTransport(pool *ConnectionPool) *ConnectionPoolTransport {
	return &ConnectionPoolTransport{
		Pool:       pool,
		Underlying: pool.Transport,
	}
}

// RoundTrip implements the http.RoundTripper interface
func (t *ConnectionPoolTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	host := req.URL.Host

	// Track active request
	t.Pool.mu.Lock()
	t.Pool.activeRequests[host]++
	t.Pool.mu.Unlock()
